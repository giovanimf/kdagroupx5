...
const Sorteio = () => {
  ...
  const handleSorteio = () => {
    // Combina jogadores já nos times e os selecionados
    const allPlayers = [...time1, ...time2, ...selecionados];

    // Valida se é possível balancear os times
    if (allPlayers.length < 2 || allPlayers.length % 2 !== 0) {
      alert("O sorteio requer um número par de jogadores para balancear os times.");
      return;
    }
    ...
    // Divide os jogadores igualmente entre os times
    const mid = Math.floor(shuffled.length / 2);
    const newTime1 = shuffled.slice(0, mid);
    const newTime2 = shuffled.slice(mid);

    // Atualiza os estados com os novos times
    setTime1(newTime1);
    setTime2(newTime2);

    // Remove os jogadores sorteados da tabela de jogadores disponíveis
    const remainingJogadores = jogadores.filter(
      (j) => !newTime1.some((t1) => t1.id === j.id) && !newTime2.some((t2) => t2.id === j.id)
    );
  ...
  const handleCriarPartida = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Redireciona para a página de edição da partida recém-criada
        router.push(`/editar-partida/${data.id}`);
      } else {
    ...
    <div>
      ...
      <DragDropContext onDragEnd={onDragEnd}>
                      ...
                      <Draggable key={jogador.id} draggableId={jogador.id.toString()} index={index}>
                        {(provided) => (
                          <div
                            ...
                            className={`p-2 mb-2 text-start rounded cursor-pointer text-black ${isSelected ? "bg-colorMintGreen" : "bg-colorSkyBlue"
                              }`}
                    ...
                    <Draggable key={jogador.id} draggableId={jogador.id.toString()} index={index}>
                      {(provided) => (
                        <div
                          ...
                          {...provided.draggableProps}
                          {...provided.dragHandleProps}
                          className="p-2 mb-2 text-black bg-colorSkyBlue text-start rounded"
                        >
                          {jogador.nome}
                          <button
        ...
        <div className="flex justify-center mt-4 space-x-4">
          <button
            className="p-2 text-black bg-green-500 font-bold rounded cursor-pointer hover:bg-colorMintGreen"
            onClick={handleSorteio}
          >
            Gerar Sorteio
          </button>
          <button
            className="p-2 text-black bg-blue-500 font-bold rounded cursor-pointer hover:bg-colorMintGreen"
            onClick={handleCriarPartida}
          >
            Criar Partida
          </button>
          <button
            className="p-2 text-black bg-orange-500 font-bold rounded cursor-pointer hover:bg-colorMintGreen"
            onClick={handleReset}
          >
            Resetar
          </button>
